...
My usual stack is Claude Code, Codex-style coding agents, repo-level context files like `CLAUDE.md` or `AGENTS.md`, GitHub MCP, and `claude-mem` MCP. GitHub MCP helps with repo-related workflows, while `claude-mem` MCP helps carry useful project context across sessions. I also use small reusable commands instead of relying on one long prompt.
