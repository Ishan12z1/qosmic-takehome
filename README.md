...
10 experiments across 5 pillars, competitor analysis, and 17 technical checks — at
the quality bar of `docs/target_report.md`.
...
artifacts are reused; `--force` starts fresh.
...
This is why the harness generalizes safely: a bot-protected store (e.g. Cloudflare)
yields an honest partial report instead of invented findings.

---

## The eval system (headline)
...
| 6 | Every evidence path exists on disk and under the correct `run_id` (cross-run contamination check) |
...
| `evals/` | `run_eval.py` (Layers 1–8, 10), `rubric.md` (Layer 9), `analyze_failures.py`, `failure_log.jsonl` |
| `CLAUDE.md`, `AGENTS.md` | Entry-point context files for the agent |
| `sample_output/` | Generated audit report(s) |
| `eval_results/` | Eval output, including the scored Layer 9 rubric |
...
> A reviewer can open `sample_output/zenrojas_20260614_ab4535_audit.md` and follow
> any citation into `artifacts/zenrojas_20260614_ab4535/` — one full verification
> bundle is committed so every cited path resolves.

---

## Sample run

`zenrojas.com` (the brief's generalization target) — `zenrojas_20260614_ab4535`:
Healthy crawl, 10/10 pages, full shopping journey (friction 5.0/5),
**9/9 deterministic eval layers** passed, **Layer 9 quality 45/50 (Excellent)**.
...
- **Cite everything.** Every audit claim resolves to an `artifacts/<run_id>/...`
  path or a URL — Layer 6 fails the run otherwise.
- **No hallucination.** A check that wasn't performed is `Warn`, never `Pass`.
