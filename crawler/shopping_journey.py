...
async def run_shopping_journey(
    ...
    pdp_entry = next((p for p in selected_pages if p["page_type"] == "product_page"), None)
    if not pdp_entry:
        ...
        checks["shopping_journey"]["detail"] = "No product page found to start journey."
        return journey
    ...
    try:
        ...
        # Detect payment methods from PDP footer
        payment_methods = detect_payment_methods(pdp_html)

        # Check for retailer routing
        retailer_detected = (
        ...
        if retailer_detected:
            ...
            journey["outcome"] = "retailer_routed"
            journey["retailer_routed"] = True
            journey["friction_score"] = 2.0
            journey["friction_flags"] = ["retailer_routed"]
            journey["friction_detail"] = "Score 2.0/5 — retailer-routed store, no direct purchase path"
        ...
        if not atc_success:
            ...
            print("  [FAIL] Add-to-cart button not found or failed")
        ...
        if not cart_ok:
            ...
            journey["journey_observations"]["cart_upsell_present"] = any(
                kw in cart_lower for kw in ["you might also", "frequently bought", "complete the look", "add on", "bundle"]
            )
            journey["journey_observations"]["urgency_elements_present"] = any(
                kw in cart_lower for kw in ["limited", "only", "left in stock", "selling fast", "hurry", "low stock"]
            )
        ...
        try:
            ...
            if "404" in checkout_lower or "page not found" in checkout_lower:
                ...
                # Set checkout_reachable pass if we got here without critical failures
